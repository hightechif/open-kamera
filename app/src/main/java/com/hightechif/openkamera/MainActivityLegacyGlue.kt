...
abstract class MainActivityLegacyGlue : AppCompatActivity(), PreferenceFragment.OnPreferenceStartFragmentCallback {
    ...
    abstract val settingsRepository: ISettingsRepository
    abstract val mediaRepository: IMediaRepository
    abstract val locationRepository: ILocationRepository
    ...
    fun initLegacy(savedInstanceState: Bundle?, debugTime: Long = 0) {
        ...
        // set up components
        bluetoothRemoteControl = BluetoothRemoteControl(mainActivity, remoteInputManager)
        permissionManager = PermissionManager(mainActivity)
        permissionHandler = permissionManager.permissionHandler
        settingsManager = SettingsManager(mainActivity)
        mainUI = MainUI(mainActivity)
        ...
        textFormatter = TextFormatter(mainActivity)
        soundPoolManager = SoundPoolManager(mainActivity)
        magneticSensor = MagneticSensor(mainActivity)

        keyEventHandler = KeyEventHandler(mainActivity)
        orientationLifecycleManager = OrientationLifecycleManager(mainActivity)
        permissionManager = PermissionManager(mainActivity)
        dialogCoordinator = DialogCoordinator(mainActivity)
        cameraLifecycleCoordinator = CameraLifecycleCoordinator(mainActivity)
        ...
        saveLocationHandler = SaveLocationHandler(mainActivity)
        if (MyDebug.LOG)
            Log.d(
                TAG,
        ...
        thread {
            textToSpeech = TextToSpeech(this@MainActivityLegacyGlue) { status ->
    ...
    private inner class MyDisplayListener : DisplayListener {
        ...
        init {
            val rotation = this@MainActivityLegacyGlue.windowManager.defaultDisplay.rotation
            if (MyDebug.LOG) {
    ...
    fun performLegacyConfigurationChanged(newConfig: Configuration) {
                    ...
                    class PhysicalCamera(val id: String) {
                        ...
                        init {
                            ...
                            this.description = preview.cameraControllerManager.getDescription(
                                info = info,
                                context = this@MainActivityLegacyGlue,
        ...
        alertDialog.setSingleChoiceItems(
            ...
            DialogInterface.OnClickListener { dialog, which ->
                ...
                if (logicalCamera in 0..<nCameras) {
                    ...
                    this@MainActivityLegacyGlue.closePopup()
                    if (preview.canSwitchCamera()) {
        ...
        override fun onSharedPreferenceChanged(sharedPreferences: SharedPreferences, key: String?) {
            ...
            when (key) {
                ...
                PreferenceKeys.WATER_TYPE -> {
                    ...
                    this@MainActivityLegacyGlue.waterDensity =
    ...
    private inner class PopupOnBackPressedCallback(enabled: Boolean) :
        ...
        override fun handleOnBackPressed() {
            ...
            if (popupIsOpen()) {
                ...
                this@MainActivityLegacyGlue.onBackPressed()
            }
        }
    }
    ...
    private inner class ScreenLockOnBackPressedCallback(enabled: Boolean) :
        ...
        override fun handleOnBackPressed() {
            ...
            if (this@MainActivityLegacyGlue.isScreenLocked) {
        ...
        run {
            ...
            decorView.rootView.setOnApplyWindowInsetsListener(object :
                ...
                override fun onApplyWindowInsets(
                    ...
                    val insetRight: Int
                    val insetBottom: Int
                    if (this@MainActivityLegacyGlue.edgeToEdgeMode && Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
                    ...
                    if (!this@MainActivityLegacyGlue.edgeToEdgeMode) {
                    ...
                    if ((this@MainActivityLegacyGlue.edgeToEdgeMode || (hasLastSystemOrientation && systemOrientation != lastSystemOrientation)) && (newNavigationGap != _navigationGap || newNavigationGapLandscape != _navigationGapLandscape || newNavigationGapReverseLandscape != _navigationGapReverseLandscape)) {
                        if (MyDebug.LOG) Log.d(
                            TAG,
                        ...
                        if (wantNoLimits || this@MainActivityLegacyGlue.edgeToEdgeMode) {
                    ...
                    } else if (!this@MainActivityLegacyGlue.edgeToEdgeMode && _navigationGap == 0) {
                        if (MyDebug.LOG) Log.d(
                            TAG,
    ...
    private fun setImmersiveTimer() {
        ...
        immersiveTimerHandler!!.postDelayed(Runnable {
            ...
            if (!this@MainActivityLegacyGlue.isAppPaused && !this@MainActivityLegacyGlue.isCameraInBackground && !popupIsOpen() && usingKitKatImmersiveMode()) setImmersiveMode(
                true
            )
    ...
    fun updateFolderHistorySAF(saveFolder: String) {
        ...
        if (saveLocationHistorySaf == null) {
            saveLocationHistorySaf =
                SaveLocationHistory(mainActivity, "save_location_history_saf", saveFolder)
        }
    ...
    fun performLegacyActivityResult(requestCode: Int, resultCode: Int, resultData: Intent?) {
        if (MyDebug.LOG) Log.d(
            TAG,
    ...
    private inner class MyGestureDetector : SimpleOnGestureListener() {
        override fun onFling(
            ...
            try {
                ...
                val vc = ViewConfiguration.get(this@MainActivityLegacyGlue)
    ...
    fun performLegacySaveInstanceState(state: Bundle) {
...
$captureRateString${resources.getString(R.string.fps)}${
    ...
    fun startAudioListener() {
        ...
        val callback = MyAudioTriggerListenerCallback(mainActivity)
    ...
    companion object {
         ...
         * with scoped storage.
         * @return [CheckSaveLocationResult] indicating validity and optional alternative path.
         * @param baseFolder This should normally be null, but can be used to specify manually the
         * folder instead of using StorageUtils.getBaseFolder() - needed for unit tests.
         */
        @JvmStatic
        @JvmOverloads
        fun checkSaveLocation(folder: String, baseFolder: String? = null): CheckSaveLocationResult {
            ...
            if (StorageUtils.saveFolderIsFull(folder)) {
                ...
                if (baseFolder!!.isNotEmpty() && baseFolder[baseFolder.length - 1] == '/') {
                    baseFolder = baseFolder.substring(0, baseFolder.length - 1)
                }
                if (MyDebug.LOG) Log.d(TAG, "    compare to base_folder: $baseFolder")
        ...
        /** Processes a user-specified save folder: strips leading/trailing slashes and
         *  collapses consecutive slashes. Use with the non-SAF scoped storage method.
         */
        @JvmStatic
        fun processUserSaveLocation(folder: String): String {
            var result = folder
            val strip = "//"
            while (result.isNotEmpty() && result.contains(strip)) {
                result = result.replace(strip.toRegex(), "/")
            }
