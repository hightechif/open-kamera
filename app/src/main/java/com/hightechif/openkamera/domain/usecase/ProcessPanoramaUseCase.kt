...
class ProcessPanoramaUseCase @Inject constructor(
    ...
    suspend operator fun invoke(
        ...
        val processResult = imageProcessor.processPanorama(frames)
        if (processResult.isFailure) {
            return Result.failure(
                processResult.exceptionOrNull()
                    ?: IllegalStateException("Panorama stitching failed")
            )
        }

        val stitchedBytes = processResult.getOrThrow()
        return mediaRepository.savePhoto(stitchedBytes, config, customFilename)
    }
}
